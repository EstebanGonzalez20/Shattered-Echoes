...
func process_mouse(event: InputEvent, input_comp: InputComponent, weapon_comp: WeaponComponent, player):
	if event is InputEventMouseMotion:
		input_comp.mouse_delta = event.relative
	if Input.is_action_pressed("basic_attack"):
		AttackSystem.perform_attack(player, weapon_comp.basic_attack)
	if Input.is_action_pressed("special_attack"):
		AttackSystem.perform_attack(player, weapon_comp.special_attack)
